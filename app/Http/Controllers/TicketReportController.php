...
class TicketReportController extends Controller
{
    public function exportExcel(Request $request)
    {
        ...
        $reportService = app(TicketReportService::class);
        $stats = $reportService->statistics($startDate, $endDate);
        ...
        // Data rows
        $row = $headerRow + 1;
        foreach ($reportService->lazyTickets($startDate, $endDate) as $ticket) {
            $resolutionDays = '-';
            ...
            $sheet->setCellValue('D'.$row, $reportService->categoryLabel($ticket->category));
            $sheet->setCellValue('E'.$row, $reportService->priorityLabel($ticket->priority));
            $sheet->setCellValue('F'.$row, $ticket->subject);
            $sheet->setCellValue('G'.$row, $reportService->statusLabel($ticket->status));
    ...
    public function exportPdf(Request $request)
    {
        ...
        $reportService = app(TicketReportService::class);
        $stats = $reportService->statistics($startDate, $endDate);
        $categoryStats = $reportService->categoryStatistics($startDate, $endDate);
        $priorityStats = $reportService->priorityStatistics($startDate, $endDate);
